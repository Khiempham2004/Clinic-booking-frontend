...
import { getAllDoctor, getAvailability } from '../../services/doctorService.js';
...
function BookingTimeSlot({ bookingData, setBookingData }) {
    ...
    const handleDoctorChange = (e) => {
        ...
        const selected = doctors.find(d => String(d._id || d.id) === String(id));
        setBookingData({ ...bookingData, doctorId: id, doctorName: selected ? (selected.fullName || selected.name || '') : '' });
    };
                        ...
                        {loadingDoctors ? (
                            ...
                            doctors.map(doc => (
                                <option key={doc._id || doc.id} value={doc._id || doc.id}>
                                    {doc.fullName || doc.name} - {doc.specialty}
                                </option>
                            ))
                        )}
                    </select>
                </div>
                {/* clinic selection removed because clinic is no longer required */}
                <div>
